...
{
	public GameObject[] pickups;				// Array of pickup prefabs with the bomb pickup first and Temperature second.
	...
	public float highHealthThreshold = 75f;		// The Temperature of the player, above which only bomb crates will be delivered.
	public float lowHealthThreshold = 25f;		// The Temperature of the player, below which only Temperature crates will be delivered.
	...
	public IEnumerator DeliverPickup()
	{
		...
		// If the player's Temperature is above the high threshold...
		if(playerHealth.Temperature >= highHealthThreshold)
			// ... instantiate a bomb pickup at the drop position.
			Instantiate(pickups[0], dropPos, Quaternion.identity);
		// Otherwise if the player's Temperature is below the low threshold...
		else if(playerHealth.Temperature <= lowHealthThreshold)
			// ... instantiate a Temperature pickup at the drop position.
